...
class BEVRender:
    def __init__(
        ...
        self.input_dir = os.path.join(out_dir, "bev_input")
    ...
    def render(
        ...
        self.draw_motion_gt(data)
        self.draw_map_gt(data)
        self.draw_planning_input(data)
        self._render_sdc_car()
        self._render_command(data)
        self._render_legend()
        save_path_input = os.path.join(self.input_dir, str(index).zfill(4) + '.jpg')

        self.save_fig(save_path_input)

        return save_path_gt, save_path_pred,save_path_input
    ...
    def draw_planning_input(self, data):
        if not self.plot_choices['planning']:
            return
        ...
        masks = data['gt_ego_fut_masks'].astype(bool)
        if masks[0] != 0:
            plan_traj = data['input_ego_fut_trajs_6dof'][:, :2][masks]
            cmd = data['gt_ego_fut_cmd']
            plan_traj[abs(plan_traj) < 0.01] = 0.0
            plan_traj = plan_traj.cumsum(axis=0)
            plan_traj = np.concatenate((np.zeros((1, plan_traj.shape[1])), plan_traj), axis=0)
    ...
    def draw_final_planning_pred(self, data, result):
        ...
        plan_traj = result['final_planning'][... , :2].cpu().numpy()
